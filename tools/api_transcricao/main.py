...
def transcrever():
    ...
    try:
        ...
        # Verifica se o tipo de conteúdo é suportado (fallback por extensão quando necessário)
        supported_types = [
            'audio/wav', 'audio/wave', 'audio/x-wav', 'audio/ogg', 'audio/mp3', 'audio/mpeg',
            'audio/mp4', 'audio/m4a', 'audio/aac', 'audio/flac'
        ]
        if not any(audio_type in content_type for audio_type in supported_types):
            # Fallback: alguns ambientes enviam application/octet-stream
            filename = None
            if 'audio' in request.files and request.files['audio']:
                filename = request.files['audio'].filename or ""
            elif 'url' in request.form and request.form['url']:
                filename = urllib.parse.urlparse(request.form['url']).path

            ext = os.path.splitext(filename)[1].lower()
            allowed_exts = ['.wav', '.ogg', '.mp3', '.mp4', '.m4a', '.aac', '.flac']
            if ext in allowed_exts:
                # força um content_type coerente para o fluxo
